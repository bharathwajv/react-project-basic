...
const FunctionalHookComponent = () => {
  ...
  const [mousePos, setMousePos] = useState({ x: null, y: null }); // can be de-structured as = const [{x,y}, setMousePos]
  const [onlineStatus, setStatus] = useState(navigator.onLine);
  const Increment = () => {
    //be only const arrow func - no 'this' is needed
    setCount(
  ...
  let isMounted = true;
  useEffect(() => {
    // runs on each render or update on any state - can be modified by second parameter to useeffect as []
    document.title = count;
    window.addEventListener("mousemove", handleMove);
    window.addEventListener("online", handleOnline);
    window.addEventListener("offline", handleOffline);
    if (isMounted) {
        //for listners which does not support disposal
    }
    return () => {
      ///when component detaches - similar to componentWillUnmount but Adv: called everytime before the effect runs
